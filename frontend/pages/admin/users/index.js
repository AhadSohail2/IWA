...
import styles from '../../../styles/admin/users/AdminUsers.module.css';
...
function AdminUsers() {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const data = await sendRequest('/admin/users', 'GET', null, {
    ...
    const deleteHandler = async (id) => {
        try {
            await sendRequest(`/admin/user/${id}`, 'DELETE', null, {
                'authorization': `Bearer ${AuthCtx.token}`
            })
